...
if (!interviewId) {
  console.error('Usage: node scripts/manual-score-interview.cjs <interviewId>');
  process.exit(1);
}
...
(async () => {
  ...
  if (interviewErr || !interview) throw new Error(interviewErr?.message || 'Interview not found');
  ...
  const normalizeText = (value) => String(value || '').replace(/\bcandidate\b/gi, (match) => {
  ...
  const rows = capabilities.map((item) => ({
    ...
    score: item.score,
    evidence_summary: normalizeText(item.evidence_summary || ''),
    strengths: (Array.isArray(item.strengths) ? item.strengths : []).map(normalizeText),
    development_areas: (Array.isArray(item.development_areas) ? item.development_areas : []).map(normalizeText),
    behavioural_patterns: Array.isArray(item.behavioural_patterns) ? item.behavioural_patterns : [],
    coaching_recommendations: (Array.isArray(item.coaching_recommendations) ? item.coaching_recommendations : []).map(normalizeText),
  }));

  if (rows.length) {
    const { error: insertErr } = await supabase.from('interview_scores').insert(rows);
    if (insertErr) throw new Error(insertErr.message);
  }

  console.log(JSON.stringify({ interviewId, scoresCreated: rows.length, overall_summary: parsed.overall_summary }, null, 2));
})();
