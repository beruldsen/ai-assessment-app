import { capabilityShortLabel, labelAnchor, orderedCapabilityRows, radarPoint, splitLabel, toneClass } from "@/lib/interviewReportUi";
    ...
    <div className="report-radar-wrap">
      <svg viewBox={`0 0 ${size} ${size}`} className="report-radar" role="img" aria-label="Capability radar chart">
        ...
        {ordered.map((item, idx) => {
          ...
          const lines = splitLabel(printMode ? capabilityShortLabel(item.capability) : item.capability, printMode ? 16 : 18, printMode ? 2 : 3);
            ...
            <g key={item.capability}>
              ...
              <text x={labelPoint.x} y={labelPoint.y} textAnchor={anchor} className="radar-label">
                {lines.map((line, lineIdx) => (
                  <tspan key={lineIdx} x={labelPoint.x} dy={lineIdx === 0 ? 0 : printMode ? 12 : 14}>{line}</tspan>
                ))}
              </text>
            </g>
          );
